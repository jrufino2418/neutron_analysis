    ...
    with open(filename, "rb") as file:
        ...
        for event in range(idx + 1):
            ...
            for i in range(waveform_length):
                ADC16 = read_exact(file, 2)
                if event == idx:
                    if i == 0:
                        x = np.array([i])
                        y = np.array([int.from_bytes(ADC16, "little", signed=False)])
                    else:
                        x = np.append(x, i)
                        y = np.append(y, int.from_bytes(ADC16, "little", signed=False))
        ...
        print(f"First 10 waveform samples (raw 16-bit): {y[:10]}")
        print("--------------------------------------------------------")
        return x, y
