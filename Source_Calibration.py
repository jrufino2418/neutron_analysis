...
integration_start, integration_offset, integration_stop, integration_method, x_min, x_max, x_bins, y_min, y_max, y_bins, upper_gate, lower_gate, slices_in_region1, slices_in_region2, total_slices = cal_setup(channel_number)
calibrate = False
...
def Light_response(PSD, long_integral_list):
    ...
    for s in range(0, total_slices,1):
        ...
        if np.sum(hist_slice) > 0:
            ...
            if result is not None:
                ...
                sigma_upper[index] = g0_center + 2*g0_sigma
                sigma_lower[index] = g0_center - 2*g0_sigma
            else:
    ...
    plt.legend()
    plt.savefig(f'ch{channel_number}/Cs137_PSD_light_response.png', dpi=300)
    return 
...
filename = f"/home/jrufino/PhD_Research/Waveform_Anaysis/ch{channel_number}/137Cs_calibration.BIN"
...
Compton_edge_calibration(long_integral_list)
#Light response fit 
